...
// handler tests. Today PATCH /me only dismisses the voice coachmark;
// the case-branched onboarding submission goes through a separate
// handler in the onboarding package.
...
func TestPatchMe_RejectsEmptyOrFalseBody(t *testing.T) {
...
func TestPatchMe_RejectsUnknownField(t *testing.T) {
	// due_date used to live on PATCH /me; today it must be rejected as an
	// unknown field so older clients get a clear 400 instead of a silent
	// pass-through.
	h, _, cleanup := newHandlersFor(t, "u1")
	defer cleanup()

	req := withUser(httptest.NewRequest(http.MethodPatch, "/me",
		strings.NewReader(`{"due_date":"2025-09-15"}`)), "u1")
	rec := httptest.NewRecorder()
	h.PatchMe(rec, req)
	if rec.Code != http.StatusBadRequest {
