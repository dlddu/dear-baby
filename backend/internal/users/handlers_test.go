...
// PATCH /me no longer accepts a due_date — the case-branching submission
// goes through POST /onboarding/case instead.
...
func TestMe_ReturnsProfile(t *testing.T) {
	...
	req := withUser(httptest.NewRequest(http.MethodGet, "/me", nil), "u1")
	rec := httptest.NewRecorder()
	h.Me(rec, req)
	if rec.Code != http.StatusOK {
		t.Fatalf("status: got %d body=%s", rec.Code, rec.Body.String())
