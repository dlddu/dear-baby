...
func TestPatchMe_InvalidBody(t *testing.T) {
	h, _, cleanup := newHandlersFor(t, "u1")
	defer cleanup()

	cases := []string{
		// Unknown field — due_date is no longer accepted on PATCH /me.
		`{"due_date":"2025-09-15"}`,
		// Mixed payload with the legacy due_date field.
		`{"due_date":"2025-09-15","dismiss_voice_coachmark":true}`,
		// dismiss_voice_coachmark must be true; explicit false is rejected.
		`{"dismiss_voice_coachmark":false}`,
		// Empty body — no actionable field present.
		`{}`,
	}
