...
// *onboarding.Store. Today the only mutation routed through PATCH /me is
// dismissing the home voice coachmark — the case-branching submission
// has its own POST /onboarding/case endpoint.
...
// patchMeBody is the request body for PATCH /me. Only one field is
// supported today — `dismiss_voice_coachmark` — kept as an object body
// so additional flag-style mutations can be added without changing the
// route.
type patchMeBody struct {
	DismissVoiceCoachmark *bool `json:"dismiss_voice_coachmark"`
}

// PatchMe updates the authenticated user's onboarding flags. Currently
// only the home-screen voice coachmark dismissal is accepted; any other
// payload returns 400.
