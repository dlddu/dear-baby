...
// patchMeBody is the request body for PATCH /me. Today the only supported
// field is `dismiss_voice_coachmark` — the legacy `due_date` path was
// retired now that onboarding completion goes through POST /onboarding/case
// (see PRD-006 AC-006-01~04). Any unknown field returns 400.
type patchMeBody struct {
	DismissVoiceCoachmark *bool `json:"dismiss_voice_coachmark"`
}

// PatchMe handles the home-screen voice coachmark dismissal. Onboarding
// completion is handled separately by POST /onboarding/case.
