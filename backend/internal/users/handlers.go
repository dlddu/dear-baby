...
// the onboarding store. Declared as an interface so the users package
// does not import the onboarding package. The router wires in the
// concrete *onboarding.Store.
//
// Today the only mutation PATCH /me makes is dismissing the home voice
// coachmark. The case-branched onboarding submission goes through POST
// /onboarding/case (a richer payload that needs its own handler).
...
// Me returns the authenticated user's profile — the flat view that
// merges users + onboarding.
...
// patchMeBody is the request body for PATCH /me. Today it only carries
// the voice-coachmark dismissal flag — onboarding completion moved to
// POST /onboarding/case.
type patchMeBody struct {
	DismissVoiceCoachmark *bool `json:"dismiss_voice_coachmark"`
}

// PatchMe updates the authenticated user's small mutable flags. The
// only field accepted today is dismiss_voice_coachmark.
