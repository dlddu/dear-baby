...
// dismissal, first_record_at, ai_preview) lives in the `onboarding`
// table and is merged into Profile for the /me response.
type User struct {
	ID         string    `json:"id"`
...
// the app can keep treating the response as a single object. The case
// branch + per-child details live in /onboarding/case responses (and
// future /me embed); Profile only carries the lightweight markers the
// home screen needs to decide between funnel and tabs.
type Profile struct {
	ID                        string     `json:"id"`
	Email                     string     `json:"email"`
	Name                      string     `json:"name"`
