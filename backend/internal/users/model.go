...
// merges users + onboarding so the client can keep treating the response
// as a single object. CaseKind is null until the user submits POST
// /onboarding/case (PRD-006 AC-006-01).
type Profile struct {
	ID                        string     `json:"id"`
	Email                     string     `json:"email"`
	Name                      string     `json:"name"`
