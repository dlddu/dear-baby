...
// the voice coachmark. The backend remains the source of truth — this
// cache is only a graceful-fallback hint.

const ONBOARDED_AT_KEY = 'db_onboarded_at';
const DUE_DATE_KEY = 'db_due_date';
const VOICE_DISMISSED_AT_KEY = 'db_voice_coachmark_dismissed_at';
...
export async function getCachedVoiceCoachmarkDismissedAt(): Promise<
  string | null
> {
  return SecureStore.getItemAsync(VOICE_DISMISSED_AT_KEY);
}
...
export async function setCachedOnboarding(
  ...
  if (voiceCoachmarkDismissedAt) {
    await SecureStore.setItemAsync(
      VOICE_DISMISSED_AT_KEY,
      voiceCoachmarkDismissedAt,
    );
  } else {
    await SecureStore.deleteItemAsync(VOICE_DISMISSED_AT_KEY);
  }
  if (firstRecordAt) {
