// User mirrors the flat profile returned by GET /me. The case-branching
// onboarding (PRD-006 AC-006-01~04) now reports its branch via
// `case_kind` ("A"|"B"|"C"); per-child details live on dedicated
// /onboarding/case responses + the children table the home screen
// fetches separately.
export type User = {
  id: string;
  email: string;
  name: string;
  picture_url: string;
  // Onboarding fields. `case_kind` is null until the user submits the
  // case-branching funnel; it co-stamps `onboarded_at`. Both nullable.
