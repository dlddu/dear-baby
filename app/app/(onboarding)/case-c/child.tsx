...
export default function CaseCChildScreen() {
  ...
  const onNext = async () => {
    ...
    if (idx + 1 < total) {
      ...
      await saveDraft({ lastStep: '/(onboarding)/case-c/purpose' });
  ...
  return (
    <OnboardingScaffold
      ...
      step={2}
      total={3}
      title={total > 1 ? `${orderName} 아이 정보` : '아이 정보를 알려주세요'}
      ctaTitle={ctaTitle}
      ctaDisabled={!ready}
      onCta={onNext}
      trailing={
        total > 1 ? (
          <RepeatBadge index={idx + 1} total={total} caseKind={'C'} />
        ) : null
      }
      testID={'onboarding-c2'}
    >
      <View style={styles.photoRow}>
        <PhotoPicker value={photo} onChange={setPhoto} testID={'c2-photo'} />
      </View>
