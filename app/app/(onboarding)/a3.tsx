// Onboarding M-06 — A3 기록 목적
// docs/mockups/source/src/screens/Onboarding.tsx:317-335 (M06_A3_Purpose)
//
// PRD-006 AC-006-02 의 마지막 입력. 사용자가 칩 8개에서 다중 선택한
// 한국어 라벨을 `OnboardingContext.purposes` 에 저장하고, [시작하기 ✨] 시
// 모든 태아 행에 동일 purposes 를 복제해 백엔드에 영속화한다.
// 다태에서도 1회만 노출 — 마지막 태아의 [다음] 에서 a2 → a3 로 진입한다.
...
// testID 에 한국어를 그대로 쓸 수 없으므로 라벨을 인덱스로 번역한다. label 을
// 직접 testID 에 넣지 않는 이유는 Maestro 에서 한국어 id 매칭이 환경마다
// 불안정하기 때문이다.
function purposeTestID(label: string): string {
  const idx = CASE_A_PURPOSES.findIndex((p) => p.label === label);
  return `onboarding-a3-purpose-${idx}`;
}

export default function OnboardingA3() {
  const router = useRouter();
  const { purposes, togglePurpose, completeAsA } = useOnboarding();
  ...
  return (
    <SafeAreaView
      style={styles.safe}
      edges={['top', 'bottom']}
      testID="onboarding-a3"
    >
      <OnboardingTopRow total={5} current={4} />
      <ScrollView
        style={styles.scroll}
      ...
      <View style={styles.actions}>
        <BackLink onPress={() => router.back()} testID="onboarding-a3-back" />
        <Button
