// A3 — 기록 목적 (복수 선택). 와이어프레임 docs/wireframes/onboarding/case-a.svg 3/3.
// 모든 태아에 동일 목적을 복제 저장한다 ([4.4]: 단순화를 위한 정책).
...
const OPTIONS: Array<{ value: RecordPurpose; title: string }> = [
  ...
  { value: 'memory_keeping', title: '임신 추억 보관' },
  { value: 'family_share', title: '가족과 공유' },
  { value: 'emotion_diary', title: '감정 일기' },
];
...
export default function CaseAPurpose() {
  ...
  useFocusEffect(
    useCallback(() => {
      void saveDraft({ last_step: '/case-a/purpose' });
    }, []),
  );
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      await clearDraft();
      // AuthGate routes to /(tabs) automatically once status flips.
    } catch (e) {
