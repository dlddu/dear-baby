...
- **V-005 (아이 단계별 맞춤 질문)**: 예정일·실제 출생일을 정확히 수집하고 출산 후 자동 갱신하여, 질문 모델이 요구하는 단계(임신 주차 → 생후 나이)를 정확하게 유지한다 (PRD-002 질문 모델)
...
- **설명**: 사용자가 아이 전환 탭으로 활성 아이를 변경하면, 홈/커뮤니티/자서전 탭이 모두 선택된 아이를 기준으로 재구성된다. 일기 탭은 예외로, 활성 아이와 무관하게 모든 아이의 기록을 통합 표시한다 (PRD-008 AC-008-01 참조)
- **달성 가치**: V-001, V-006
- **조건**:
  - **홈 탭**: 선택된 아이의 주차/일수, 오늘의 질문, 최근 기록 미리보기가 해당 아이 기준으로 표시된다
  - **일기 탭**: **활성 아이 컨텍스트와 무관하게** 사용자의 모든 아이 기록을 통합 표시한다. 다자녀 사용자가 특정 아이만 보고 싶을 때는 일기 탭 내부의 아이 필터를 사용한다 (구체 명세는 [PRD-008](PRD-008-diary-tab.md))
