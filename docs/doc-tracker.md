...
- 사용자 여정: **7개** (Onboarding · Daily Recording · Birth Conversion · AI Narrative · Book Production · Diary Browsing · **Login & Session**) — Login & Session 은 **PRD 없이 ENG-003·ENG-006 에 매핑되는 유일한 여정**(인증을 PRD/AC 밖에 두는 아래 e2e 매핑 방침과 동일 근거). 대응 mockup M-44~M-50 은 여정 문서가 요구사항으로 정의만 해둔 상태이며 **후속 PR에서 신설 예정**
- 흐름도: **1개** (onboarding-flow.md)
- Mockup: **43개 화면** (M-01 ~ M-43, 일부 결번) + 갤러리, 단일 React 번들 `docs/index.html` (GitHub Pages 진입점). M-01~M-42 는 사용자 여정 stage 1:1 매핑 / **M-43(커뮤니티 메인)은 여정 문서 부재로 PRD-009 AC 직접 매핑**
...
|------|-----------|-----------|-----------|
| 2026-08-15 로그인·세션 여정 신설 | **7번째 사용자 여정 `journeys/login-journey.md` 신설** — 인증을 사용자 관점으로 처음 문서화했다. 배경: 로그인의 실제 동작은 ENG-003·ENG-006 에 상세히 기술돼 있으나, 여정 문서 측에는 온보딩 Stage 2(가입) 표 한 줄뿐이라 **재방문·인증 실패·세션 만료·로그아웃 전 구간이 사용자 관점에서 미기술**이었다. ① **L1~L7 + 부록 stage 정의** — L1 자동 로그인 · L2 로그인 선택 · L3 인증 실패 · L4 서버 연결 불가 · L5 세션 유지 · L6 세션 만료 · L7 로그아웃, 부록은 테스터 로그인(일반 사용자 여정이 아닌 심사·E2E 운영 경로라 분리). ② **핵심 프레이밍** — L1·L5 는 사용자가 알아채지 못해야 성공(무감정 = 성공)이고 L3·L6 은 알아채게 만들어야 성공인 구간이다. 현재 구현은 전자는 충족, 후자가 비어 있다. 감정 곡선의 두 골이 정확히 L3·L6 에 파인다. ③ **ENG 관찰 항목을 사용자 관점 페인포인트로 승격** — ENG-003 §10-1(소셜 로그인 실패 완전 무반응)·§10-4(약관 링크 미동작)·§5(세션 만료 축출에 설명 없음)·ENG-006 §8-1(온보딩 draft 가 로그아웃 시 잔존 → 다음 계정에 hydrate, 공용 기기 프라이버시)를 8행 페인포인트 표로 등재. ④ **PRD 1:1 매핑 원칙의 예외를 명문화** — 본 여정만 PRD/AC 가 아니라 ENG-003·ENG-006 에 매핑된다(인증은 제품 기능이 아니라 기록 귀속·세션 유지의 기반이라는 기존 e2e 매핑 방침과 동일 근거). 대응 TEST 문서도 두지 않는다. 이 예외를 문서 본문·journeys/README 분할 원칙 양쪽에 적어 **고아 여정 오독을 차단**. ⑤ **가치 매핑** — V-002 지배적이나 L6·L7 에서는 V-001 이 전면에 나온다(세션이 끊길 때 사용자가 걱정하는 것은 로그인이 아니라 기록의 존속이며, 그래서 만료·로그아웃 카피는 인증이 아니라 기록을 이야기한다). ⑥ **경계 정리** — 온보딩 Stage 2 에 상호 참조를 넣어 두 여정이 M-01 을 공유하되 다루는 구간이 다름을 명시하고, 온보딩의 "앞 플로우 인계 없음" 을 로그인 여정 L2 로부터의 인계로 정정. journeys/README 에 7행·가치 매핑·페인포인트 요약·기반 루프 성격(순차 플로우가 아님) 반영. ⑦ **대응 mockup M-44~M-50 은 본 문서가 요구사항만 정의**하고 실물은 후속 PR — "아직 없는 화면" 절에 7개 예정 ID·stage·현재 구현 유무를 표로 두어 죽은 참조를 막았다. 문서 전용 변경 — mockup 집계(43)·AC·e2e 매핑 무변동. | 여정 6개, 인증은 ENG-003·006 에만 기술 (사용자 관점 미기술), 온보딩 Stage 2 표 1행 | 여정 **7개**, 로그인·세션 사용자 관점 전 구간 기술, 페인포인트 8건 등재(🔴 3건), mockup 7종 요구사항 정의(실물 후속) |
