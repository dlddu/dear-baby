...
- Acceptance Criteria: **68개** (확정 55개 + PRD-009 draft 13개, 모두 가치 연결됨) — 2026-08-05: PRD-001 +2 신설(AC-001-06 공개 설정·AC-001-07 임시 저장), PRD-007 -2 이관(AC-007-08→PRD-009 예정·AC-007-09→ENG)으로 총계 불변
...
- 용어집: **1개** (GLOSSARY-001)
- e2e flow (Maestro): AC↔flow 1:1 매핑 확정 — 제품 flow 20개(각 AC 1개 단독) + 후속 flow 35건(미작성) + 과도기 1건(`home-feed.yaml` — AC-007-08 이관으로 PRD-009 확정 시 AC-009-14 재매핑 대기). **영구 예외 AC 0건** — 자동화 곤란은 AC가 아니라 외부 호출 모킹 경계 6개(2 구현·4 신설)로 등재(경계만 결정적 치환, 나머지 구간은 실 e2e). + 엔지니어링 노트 1개(`login.yaml`→ENG-003) + 스모크 예외 1개(`health.yaml`). 상세는 "e2e flow 매핑" 섹션 참조
- **건강 상태**: ⚠️ 주의 — **PRD-009 (커뮤니티 탭)가 draft 상태로 편입됨**. 원문(0704 최서영 WIP 기획안)이 작업 중 문서라 AC 13개가 미확정이며, TEST-009·커뮤니티 여정·mockup이 아직 없다. 확정 계층(PRD-001~008 · AC 55 · TEST 8)은 여전히 ✅ 건강하며, 확정 AC 55의 e2e AC↔flow 매핑을 1:1 구조로 정리(제품 flow 21 · 후속 flow 34 미작성 · 영구 예외 AC 0 → 외부 호출 모킹 경계 6개로 등재 · `login.yaml`→ENG-003 · `health.yaml` 스모크 예외) — 상세는 "e2e flow 매핑" 섹션 참조
...
| V-008 공감을 통한 연결 | abstract | PRD-001, PRD-007, PRD-009(draft) | ✅ |
...
| PRD-007 홈 화면 구성 | 8 (+2 이관) | TEST-007 (17 TC) | ✅ — TC-007-08·09-A·09-B 3건은 PRD-009 확정 시 TEST-009로 이관 예정 |
...
| `home-feed.yaml` | 과도기 | (구 AC-007-08) | 명세가 PRD-009 AC-009-14로 이관되는 과도기 flow. PRD-009 확정 시 헤더 재선언·재매핑, 그때까지 CI 유지 |
...
> 2026-07-12 첫 정비 · 2026-07-15 예외 재정비 · 2026-07-20 경계 재프레이밍 · 2026-07-22 AC-007-01 flow 신설 · **2026-07-27 AC-008-10 flow 신설** · **2026-07-30 AC-007-05 flow 신설** · **2026-08-05 PRD-001 AC 2건 신설 + AC-007-08/09 이관** 기준: ✅ 20 · ⬜ 35 · 🟡 0 · 과도기 1(`home-feed.yaml`). 모든 제품 flow 는 헤더에 정확히 1개 AC 를 선언한다(중복·묶음 0).
...
| AC-007-08 | 타인 기록 피드 — *(PRD-009 AC-009-14로 이관, 2026-08-05)* | ➡️ `home-feed.yaml` 과도기 보유 — PRD-009 확정 시 재매핑 |
