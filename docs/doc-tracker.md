...
- 용어집: **1개** (GLOSSARY-001)
- e2e flow (Maestro): AC↔flow 1:1 매핑 확정 — 제품 flow 18개(각 AC 1개 단독) + 예외 등재 7건 + 후속 flow 30건(미작성) + 엔지니어링 노트 1개(`login.yaml`→ENG-003) + 스모크 예외 1개(`health.yaml`). 상세는 "e2e flow 매핑" 섹션 참조
...
> 2026-07-12 첫 정비 · 2026-07-15 예외 재정비 기준: ✅ 18 · 🟡 7 · ⬜ 30. 모든 제품 flow 는 헤더에 정확히 1개 AC 를 선언한다(중복·묶음 0).
...
**🟡 예외 상세** (7건 — e2e 로 **본질적으로 구동 불가**한 것만 / 사유 / 대체 검증):
...
- **AC-005-08** 미디어 AI 서사 연동 — 미디어→AI 서사 연동 **산출 품질**은 e2e 불가. 대체: 연동 로직 단위 테스트 + 수동 QA.
- **AC-006-05** 출산 D-7 푸시 알림 — 푸시 알림 실발송은 e2e 불가. 대체: 스케줄러 백엔드 단위 테스트 + 수동 QA.

> **2026-07-15 예외 재정비** (reconciler rct_20260712-0001 재계획): 예외를 "본질적으로 e2e 구동 불가"로 한정. 5건을 후속 flow 로 재분류 — **AC-001-02**(canned-transcript fixture 로 변환결과 표시 검증 가능), **AC-003-04**(순서변경·제외·캡션 = 결정적 편집 UI), **AC-004-04**(상태표시 = 스텁 백엔드 검증 가능), **AC-007-09**(공개만·본인제외·비식별 단정 가능; 랜덤성만 후속), **AC-002-02**(매칭 알고리즘 미확정 → blocked backlog).

**⬜ 후속 flow backlog** (30건 — 목표 flow / 단정 핵심; 새 maestro flow 는 앱 구동 검증이 필요해 후속 task 로 저작):
