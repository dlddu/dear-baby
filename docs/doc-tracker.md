...
- Mockup: **43개 화면** (M-01 ~ M-43, 일부 결번) + 갤러리, 단일 React 번들 `docs/mockups/index.html` (Pages 진입점은 문서 포털 허브 `docs/index.html`). M-01~M-42 는 6개 사용자 여정의 진입점(갤러리 그룹 5 + 독립 여정 페이지 1)에 전수 매핑(떠 있는 카드 0장) / **M-43(커뮤니티 메인)은 여정 문서 부재로 PRD-009 AC 직접 매핑** — 단계 수준 매핑·미시각화·구 번호 라벨 원장은 [여정 ↔ 목업 매핑 레지스트리](journey-mockup-map.md) 참조
...
- 용어집: **1개** (GLOSSARY-001)
- e2e flow (Maestro): **판정 축이 AC ↔ flow 에서 테스트 시나리오(TC) ↔ flow 로 전환됨** (2026-09-21). 테스트 시나리오 **112개** 대비 — 매칭 flow **22개**(각 TC 1개 단독 선언) · 영구 예외 **0건** · 구현 대기 **58건**(자서전·미디어·출산 전환/설정·알림·단계별 질문 풀·커뮤니티 후속 등, 시나리오별 코드 근거 등재) · **공백 32건**(구현됐으나 전용 flow 미작성 = 잔존 gap, 시나리오별 관측 대상 등재). 불변식 (112 − 0 − 58) = 54 ≠ 22 로 **아직 성립하지 않으며**, 그 차이가 곧 공백 32건이다. 자동화 곤란은 시나리오가 아니라 외부 호출 모킹 경계 6개(2 구현·4 신설)로 등재한다. + 엔지니어링 노트 1개(`login.yaml`→ENG-003) + 스모크 예외 1개(`health.yaml`). 상세·전수 목록은 "e2e flow 매핑" 섹션, 기계 검증은 `e2e/scripts/check-scenario-mapping.sh`
...
| PRD-007 홈 화면 구성 | 8 | TEST-007 (15 TC) | ✅ — TC-007-08·09-A/B 3건은 TEST-009로 이관 완료 (2026-08-05) |
| PRD-008 일기 탭 — 내 기록 조회 및 관리 | 10 | TEST-008 (26 TC) | ✅ |
| PRD-009 커뮤니티 탭 | 14 (검증 대상 13 + 1차 제외 1) | TEST-009 (18 TC) | ✅ — AC-009-12는 1차 제외로 TC 없음 |

## e2e flow 매핑

e2e flow(`e2e/maestro/*.yaml` **최상위**)는 다음 중 하나에 매핑되어야 하며, 셋 다 아니면 고아로 간주한다.

1. **테스트 시나리오** — `docs/tests/`의 `### TC-NNN-NN` 테스트 케이스를 검증하는 flow (대부분)
...
**등재 규칙**: (1) 시나리오를 검증하는 flow 는 헤더에 **`검증 시나리오: TC-NNN-NN` 주석을 정확히 1줄** 두고(그 아래 `상위 AC:` 로 소속 AC 를 병기), (2) 아래 매핑 표에 등재한다. 유형 (2)·(3) 의 비-시나리오 flow 는 시나리오를 갖지 않으므로 **`검증 대상:` 키를 그대로 쓴다** — 두 키의 파일 집합은 서로소이며, 그래서 `검증 시나리오:` 줄 수가 곧 매칭 파일 수다. `subflows/*.yaml` 은 테스트가 아니라 재사용 헬퍼이므로 매핑 대상이 아니다.

### 테스트 시나리오 ↔ e2e flow 매핑

> **판정 단위는 AC 가 아니라 테스트 시나리오다.** `docs/tests/`의 `### TC-NNN-NN: …` 헤딩 하나가 한 단위이고, `TC-NNN-NN` 은 문서 위치와 무관한 **안정 ID**(서수가 아니다)다. 각 TC 는 자기 문서 안에서 `- **검증 대상**: AC-…` 로 소속 AC 를 스스로 선언하므로, AC 커버리지는 아래 표의 "상위 AC" 열에서 파생된다 — 별도의 AC↔flow 표를 두지 않는다(선언하는 주체가 없는 표는 반드시 낡는다).
>
> **판정은 개수 독립적 불변식으로 한다**: (시나리오 수 − 예외 수 − 구현 대기 수) = (시나리오 매칭 파일 수). 아래 집계는 그 불변식의 현재 값이며, **지금은 성립하지 않는다** — 부족분이 곧 잔존 gap 이다.
>
> **2026-09-21 축 전환**(reconciler `rct_20260908-0001`): 종래 `AC ↔ e2e flow 매핑` 표를 대체했다. 전환 근거와 이관 내역은 아래 "변경 이력" 참조.

| TC | 시나리오 | 매칭 flow | 상위 AC |
|----|----------|-----------|---------|
| `TC-001-01` | 음성 녹음 시작 및 종료 테스트 | ✅ `home-voice-record.yaml` | AC-001-01 |
| `TC-002-01-A` | 오늘의 질문 표시 — 임산부 | ✅ `daily-question-display.yaml` | AC-002-01 |
| `TC-006-04` | Case A — 단태 입력 후 홈 진입 | ✅ `onboarding-caseA.yaml` | AC-006-02 |
| `TC-006-05` | Case B — 양육 → 임신 순서로 입력 진행 | ✅ `onboarding-caseB.yaml` | AC-006-03 |
| `TC-006-06` | Case C — 다자녀 양육자 입력 | ✅ `onboarding-caseC.yaml` | AC-006-04 |
| `TC-007-01-A` | 임산부 — 28주차 헤더 표시 | ✅ `home-header-active-child.yaml` | AC-007-01 |
| `TC-007-02-B` | 다자녀 — 아이 전환 시 화면 갱신 | ✅ `home-header-multichild.yaml` | AC-007-02 |
| `TC-007-03` | 알림 아이콘 red dot | ✅ `home-header-notification-dot.yaml` | AC-007-03 |
| `TC-007-04` | 오늘의 질문 카드 표시 | ✅ `home-question-card-undecided.yaml` | AC-007-04 |
| `TC-007-05` | 다른 질문 보기 (회전 3개) | ✅ `home-question-reroll.yaml` | AC-007-05 |
| `TC-007-06-A` | 음성 기록 진입 | ✅ `home-record-ctas.yaml` | AC-007-06 |
| `TC-007-07-A` | 책 진행도 표시 (n < 50) | ✅ `home-book-progress.yaml` | AC-007-07 |
| `TC-007-10` | 하단 네비게이션 탭 이동 | ✅ `home-nav-5tabs.yaml` | AC-007-10 |
| `TC-008-01-B` | 다자녀 — 모든 아이의 기록을 한 목록에 통합 | ✅ `diary-list-multichild.yaml` | AC-008-01 |
| `TC-008-04-A` | 기록 상세 진입과 본문 표시 | ✅ `diary-detail-and-back.yaml` | AC-008-04 |
| `TC-008-05-A` | 사후 편집 — 본문 텍스트 수정 | ✅ `diary-edit.yaml` | AC-008-05 |
| `TC-008-06-A` | 삭제 확인 모달 — 아이 이름 치환 | ✅ `diary-delete-confirm.yaml` | AC-008-06 |
| `TC-008-07-A` | 비공개 → 공개 전환 (상세 화면 액션) | ✅ `diary-visibility-toggle.yaml` | AC-008-07 |
| `TC-008-08-A` | 아이 필터 — 다자녀 다중 선택 | ✅ `diary-filter-by-child.yaml` | AC-008-08 |
| `TC-008-09-A` | 기록 0건 빈 상태 | ✅ `diary-list-empty.yaml` | AC-008-09 |
| `TC-008-10-A` | 일기 탭 헤더 구성 | ✅ `diary-header-nav.yaml` | AC-008-10 |
| `TC-009-14-B` | 홈 섹션 — 빈 상태 (mock 없음) | ✅ `home-feed.yaml` | AC-009-14 |

**매칭 22건** — 제품 flow 22개가 각각 실재 시나리오 1개를 단독 선언한다(중복 0·묶음 0·헤더 누락 0). 비-시나리오 허용 유형 2개(`login.yaml`·`health.yaml`)는 위 유형 표 소관이라 이 수에 들어가지 않는다.

### 예외 목록 (영구 면제)

<!-- e2e-exceptions:begin -->
...
**예외 0건.** 2026-07-20 경계 재프레이밍 이래 이 레포는 "자동 검증 곤란"을 **시나리오에 붙이지 않는다** — 외부 서비스 의존은 해당 외부 호출 경계만 결정적으로 치환하면 나머지 구간이 실제로 구동되므로, 아래 [외부 호출 모킹 경계 레지스트리](#외부-호출-모킹-경계-레지스트리-mock-boundary-registry)에 **경계(라이브러리/서비스) 단위**로 등재한다. 축 전환에서도 이 입장을 그대로 승계했다. 어떤 시나리오가 경계를 치환하고도 여전히 구동 불가임이 밝혀지면 **그때 이 표에 사유와 대체 검증 수단을 적어 등재**한다.

### 구현 대기 (임시 — 관측 대상 미구현)

> **"아직 구현되지 않았다"는 예외 사유가 아니다.** 기능이 없으면 e2e 가 관측할 대상 자체가 없으므로 매핑 누락(drift)이 아니라 임시 보류다. 구현이 착지하면 `docs/tests` 트리 해시가 아니라 **구현 쪽 변경으로 재감지가 돌아** 자동으로 1:1 판정 대상으로 복귀한다. **이 루프는 미구현을 해소하지 않는다** — 구현은 `tbm_dear-baby-docs-impl` 의 몫이다.
>
> **등재 기준(2026-09-21 전수 판정)**: 시나리오의 **테스트 단계 중 하나라도** 앱·백엔드에 존재하지 않는 화면·기능·API 를 요구하면 여기 둔다(근거 열에 그 부재의 코드 위치를 적는다). 전 단계가 현재 코드로 관측 가능하면 아래 공백 목록(drift)이다. 부분 구현(일부 단계만 관측 가능)은 여기 두되 근거에 어느 단계가 막히는지 적는다 — 시나리오는 쪼개지 않는다(테스트 문서 개정은 이 루프의 몫이 아니다).

<!-- e2e-pending:begin -->

| TC | 시나리오 | 미구현 근거 (코드 실측, main `1c7a1cd`) | 담당 | 해제 조건 |
|----|----------|------------------------------------------|------|-----------|
| `TC-001-06` | 작성 시점 공개/비공개 선택 테스트 | 저장 화면에 공개/비공개 토글이 없다 — `app/app/record-text.tsx`·`record-audio-review.tsx` 는 본문 입력 + 저장/취소만 렌더(`visibility` 참조 0건). 기능 PR #170(`feat(app): 기록 저장 시점 공개/비공개 토글`)이 열려 있으나 미착지 | `tbm_dear-baby-docs-impl` (PRD-001) | #170(또는 동등 구현) 착지 — 착지 시 그 PR 의 flow 헤더를 `검증 시나리오: TC-001-06` 으로 선언 |
| `TC-002-02-A` | 단계별 질문 매칭 — 임신 축 | 단계별 질문 풀·회전 알고리즘 미구현 — `app/src/data/dailyQuestions.ts` 머리말이 「본격적인 주차별·모드별 질문 풀과 회전 알고리즘은 PRD-002 후속 작업」이라 적고 12개 임시 풀에서 날짜 결정적으로 3개를 고른다(주차·개월·축 무관) | `tbm_dear-baby-docs-impl` (PRD-002) | 단계(임신 주차/생후 나이) 축별 질문 풀 구현 착지 |
| `TC-002-02-B` | 단계별 질문 매칭 — 양육 축 | 단계별 질문 풀·회전 알고리즘 미구현 — `app/src/data/dailyQuestions.ts` 머리말이 「본격적인 주차별·모드별 질문 풀과 회전 알고리즘은 PRD-002 후속 작업」이라 적고 12개 임시 풀에서 날짜 결정적으로 3개를 고른다(주차·개월·축 무관) | `tbm_dear-baby-docs-impl` (PRD-002) | 단계(임신 주차/생후 나이) 축별 질문 풀 구현 착지 |
| `TC-002-02-C` | 두 풀의 분리 — 케이스 혼합 금지 | 단계별 질문 풀·회전 알고리즘 미구현 — `app/src/data/dailyQuestions.ts` 머리말이 「본격적인 주차별·모드별 질문 풀과 회전 알고리즘은 PRD-002 후속 작업」이라 적고 12개 임시 풀에서 날짜 결정적으로 3개를 고른다(주차·개월·축 무관) | `tbm_dear-baby-docs-impl` (PRD-002) | 단계(임신 주차/생후 나이) 축별 질문 풀 구현 착지 |
| `TC-002-03-B` | 단계 스냅샷의 불변성 | 출산 전환(AC-006-06) 미구현 — 앱·백엔드에 태아→아이 전환 경로가 없다(`backend/internal/app/router.go` 에 subject 전환·수정 라우트 0건, 앱 전역 「태어났」 문자열 0건). 2단계 「출산 전환을 수행한다」를 관측할 대상이 없다 | `tbm_dear-baby-docs-impl` (PRD-006) | 출산 확인 팝업·전환(AC-006-06) 착지 |
| `TC-002-03-C` | 기준값 정정 시 스냅샷 재계산 | 기준값(예정일) 정정 UI·API 미구현 — `app/app/(tabs)/settings.tsx` 는 로그아웃 버튼만 있고(72줄), 백엔드에 subject PATCH 라우트가 없다. 재계산 자체(ENG-013, #175)는 백엔드에 착지했으나 트리거 T1 을 사용자가 일으킬 수단이 없다 | `tbm_dear-baby-docs-impl` (PRD-002 / PRD-006) | 설정에서 예정일·생년월일 수정 착지 |
| `TC-002-04` | 질문 알림 발송 테스트 | 질문 푸시 알림 미구현 — `app/package.json` 에 `expo-notifications` 없음, 앱·백엔드에 알림 발송·시간 설정 코드 0건 | `tbm_dear-baby-docs-impl` (PRD-002) | 질문 알림 발송·시간 설정 착지 (OS 푸시 실도달은 착지 뒤 모킹 경계 등재 대상) |
| `TC-002-05-B` | 기준값 입력 후 단계별 풀 복귀 | 설정에서 예정일 입력 미구현 — `settings.tsx` 로그아웃 전용, subject PATCH 라우트 없음(TC-002-03-C 와 같은 부재) | `tbm_dear-baby-docs-impl` (PRD-002 / PRD-006) | 설정에서 예정일 입력 착지 |
| `TC-003-01` | AI 서사 생성 테스트 | AI 서사 화면 미구현 — 자서전 탭 `app/app/(tabs)/memoir.tsx` 가 「곧 추가됩니다」 29줄 플레이스홀더, 백엔드 라우터에 서사 생성·조회 라우트 0건 | `tbm_dear-baby-docs-impl` (PRD-003) | 자서전 탭(서사 생성·미리보기·편집) 착지 |
| `TC-003-02` | 서사 미리보기 테스트 | AI 서사 화면 미구현 — 자서전 탭 `app/app/(tabs)/memoir.tsx` 가 「곧 추가됩니다」 29줄 플레이스홀더, 백엔드 라우터에 서사 생성·조회 라우트 0건 | `tbm_dear-baby-docs-impl` (PRD-003) | 자서전 탭(서사 생성·미리보기·편집) 착지 |
| `TC-003-03` | 서사 편집 테스트 | AI 서사 화면 미구현 — 자서전 탭 `app/app/(tabs)/memoir.tsx` 가 「곧 추가됩니다」 29줄 플레이스홀더, 백엔드 라우터에 서사 생성·조회 라우트 0건 | `tbm_dear-baby-docs-impl` (PRD-003) | 자서전 탭(서사 생성·미리보기·편집) 착지 |
| `TC-003-04` | 미디어 서사 통합 테스트 | AI 서사 화면 미구현 — 자서전 탭 `app/app/(tabs)/memoir.tsx` 가 「곧 추가됩니다」 29줄 플레이스홀더, 백엔드 라우터에 서사 생성·조회 라우트 0건 | `tbm_dear-baby-docs-impl` (PRD-003) | 자서전 탭(서사 생성·미리보기·편집) 착지 |
| `TC-004-01` | 책 레이아웃 선택 테스트 | 책 제작 화면 미구현 — 자서전 탭 플레이스홀더(위와 동일), 주문·결제·배송 추적 코드 0건 | `tbm_dear-baby-docs-impl` (PRD-004) | 책 제작 흐름 착지 (실결제·실물 배송은 착지 뒤 모킹 경계 등재 대상) |
| `TC-004-02` | 책 내용 구성 확인 테스트 | 책 제작 화면 미구현 — 자서전 탭 플레이스홀더(위와 동일), 주문·결제·배송 추적 코드 0건 | `tbm_dear-baby-docs-impl` (PRD-004) | 책 제작 흐름 착지 (실결제·실물 배송은 착지 뒤 모킹 경계 등재 대상) |
| `TC-004-03` | 제작 주문 및 결제 테스트 | 책 제작 화면 미구현 — 자서전 탭 플레이스홀더(위와 동일), 주문·결제·배송 추적 코드 0건 | `tbm_dear-baby-docs-impl` (PRD-004) | 책 제작 흐름 착지 (실결제·실물 배송은 착지 뒤 모킹 경계 등재 대상) |
| `TC-004-04` | 제작 상태 추적 테스트 | 책 제작 화면 미구현 — 자서전 탭 플레이스홀더(위와 동일), 주문·결제·배송 추적 코드 0건 | `tbm_dear-baby-docs-impl` (PRD-004) | 책 제작 흐름 착지 (실결제·실물 배송은 착지 뒤 모킹 경계 등재 대상) |
| `TC-005-01` | 사진 촬영 첨부 테스트 | 사진·영상 첨부 미구현 — 앱에 `expo-image-picker`·`expo-camera` 의존성 없음, 기록 화면에 첨부 UI 0건, 백엔드 미디어 라우트는 `records/{id}/audio/upload-url` 하나뿐 | `tbm_dear-baby-docs-impl` (PRD-005) | 미디어 첨부(촬영·갤러리·미리보기·관리) 착지 |
| `TC-005-02` | 갤러리 사진 첨부 테스트 | 사진·영상 첨부 미구현 — 앱에 `expo-image-picker`·`expo-camera` 의존성 없음, 기록 화면에 첨부 UI 0건, 백엔드 미디어 라우트는 `records/{id}/audio/upload-url` 하나뿐 | `tbm_dear-baby-docs-impl` (PRD-005) | 미디어 첨부(촬영·갤러리·미리보기·관리) 착지 |
| `TC-005-03` | 영상 촬영 첨부 테스트 | 사진·영상 첨부 미구현 — 앱에 `expo-image-picker`·`expo-camera` 의존성 없음, 기록 화면에 첨부 UI 0건, 백엔드 미디어 라우트는 `records/{id}/audio/upload-url` 하나뿐 | `tbm_dear-baby-docs-impl` (PRD-005) | 미디어 첨부(촬영·갤러리·미리보기·관리) 착지 |
| `TC-005-04` | 갤러리 영상 첨부 테스트 | 사진·영상 첨부 미구현 — 앱에 `expo-image-picker`·`expo-camera` 의존성 없음, 기록 화면에 첨부 UI 0건, 백엔드 미디어 라우트는 `records/{id}/audio/upload-url` 하나뿐 | `tbm_dear-baby-docs-impl` (PRD-005) | 미디어 첨부(촬영·갤러리·미리보기·관리) 착지 |
| `TC-005-05` | 음성 메모 첨부 테스트 | 음성 메모 재생 미구현 — 음성 원본은 저장되지만(`audio_s3_key`) 상세 `app/app/diary/[id].tsx` 는 「🎙️ 음성 원본 있음」 캡션만 그리고 재생 컨트롤이 없다 | `tbm_dear-baby-docs-impl` (PRD-005) | 기록 상세 음성 재생 컨트롤 착지 |
| `TC-005-06` | 미디어 미리보기 및 관리 테스트 | 사진·영상 첨부 미구현 — 앱에 `expo-image-picker`·`expo-camera` 의존성 없음, 기록 화면에 첨부 UI 0건, 백엔드 미디어 라우트는 `records/{id}/audio/upload-url` 하나뿐 | `tbm_dear-baby-docs-impl` (PRD-005) | 미디어 첨부(촬영·갤러리·미리보기·관리) 착지 |
| `TC-005-07` | 미디어 포함 기록 목록 표시 테스트 | 사진·영상 첨부 미구현 — 앱에 `expo-image-picker`·`expo-camera` 의존성 없음, 기록 화면에 첨부 UI 0건, 백엔드 미디어 라우트는 `records/{id}/audio/upload-url` 하나뿐 | `tbm_dear-baby-docs-impl` (PRD-005) | 미디어 첨부(촬영·갤러리·미리보기·관리) 착지 |
| `TC-005-08` | 미디어 AI 서사 연동 테스트 | 미디어 첨부·AI 서사 둘 다 미구현(위 두 부재의 합) | `tbm_dear-baby-docs-impl` (PRD-005 / PRD-003) | 미디어 첨부 + 자서전 탭 착지 |
| `TC-006-07` | 출산 D-7 푸시 알림 발송 | 출산 D-7 푸시 미구현 — 알림 의존성·스케줄러 0건(TC-002-04 와 같은 부재) | `tbm_dear-baby-docs-impl` (PRD-006) | D-7 알림 스케줄러 착지 |
| `TC-006-08` | 출산 확인 팝업 — [네, 태어났어요] 선택 시 양육자 모드 전환 | 출산 확인 팝업·양육자 모드 전환 미구현 — 앱 전역 「태어났」·「출산 확인」 문자열 0건, 백엔드 subject 전환 라우트 0건, 설정 탭에 배너 없음 | `tbm_dear-baby-docs-impl` (PRD-006) | 출산 확인 팝업 + 전환(AC-006-06·07) 착지 |
| `TC-006-09` | 출산 확인 팝업 — 다태아 출생일 개별 입력 | 출산 확인 팝업·양육자 모드 전환 미구현 — 앱 전역 「태어났」·「출산 확인」 문자열 0건, 백엔드 subject 전환 라우트 0건, 설정 탭에 배너 없음 | `tbm_dear-baby-docs-impl` (PRD-006) | 출산 확인 팝업 + 전환(AC-006-06·07) 착지 |
| `TC-006-10` | 출산 확인 팝업 — [아직이에요] 선택 시 1주 후 재질문 | 출산 확인 팝업·양육자 모드 전환 미구현 — 앱 전역 「태어났」·「출산 확인」 문자열 0건, 백엔드 subject 전환 라우트 0건, 설정 탭에 배너 없음 | `tbm_dear-baby-docs-impl` (PRD-006) | 출산 확인 팝업 + 전환(AC-006-06·07) 착지 |
| `TC-006-11` | 출산 D+14 이후 자동 팝업 중단 및 설정 배너 노출 | 출산 확인 팝업·양육자 모드 전환 미구현 — 앱 전역 「태어났」·「출산 확인」 문자열 0건, 백엔드 subject 전환 라우트 0건, 설정 탭에 배너 없음 | `tbm_dear-baby-docs-impl` (PRD-006) | 출산 확인 팝업 + 전환(AC-006-06·07) 착지 |
| `TC-006-14` | 다자녀 — 활성 아이 기준으로 모든 탭 재구성 | 3단계 「자서전 탭에서 활성 아이의 책」을 관측할 대상이 없다 — `memoir.tsx` 플레이스홀더. 일기·커뮤니티 탭의 재구성(2·6단계)은 구현돼 있으나 시나리오 전 단계가 한 flow 로 닫히지 않는다 | `tbm_dear-baby-docs-impl` (PRD-006 / PRD-003) | 자서전 탭 착지 |
| `TC-006-16` | 다자녀 — 새 기록 작성 시 기본 대상 아이가 활성 아이 | 3단계 「대상 아이를 다른 아이로 변경」 UI 미구현 — `record-text.tsx`·`record-audio.tsx` 는 `activeChild.subjectId` 를 그대로 쓰고 선택 UI 가 없다(2단계 기본값은 구현됨) | `tbm_dear-baby-docs-impl` (PRD-006) | 기록 작성 화면의 대상 아이 선택 UI 착지 |
| `TC-006-17` | 설정 탭 — 임신 중 아이 추가 (우선순위 낮음) | 설정 탭 「아이 추가」 미구현 — `settings.tsx` 로그아웃 전용, 백엔드에 온보딩 이후 subject 추가 라우트 0건(`/me/onboarding/case-*` 뿐) | `tbm_dear-baby-docs-impl` (PRD-006) | 설정 아이 추가 흐름 착지 |
| `TC-006-18` | 설정 탭 — 양육 중 아이 추가 (우선순위 낮음) | 설정 탭 「아이 추가」 미구현 — `settings.tsx` 로그아웃 전용, 백엔드에 온보딩 이후 subject 추가 라우트 0건(`/me/onboarding/case-*` 뿐) | `tbm_dear-baby-docs-impl` (PRD-006) | 설정 아이 추가 흐름 착지 |
| `TC-006-19` | 푸시 권한 미허용 시 인앱 팝업은 정상 동작 | 출산 확인 팝업·양육자 모드 전환 미구현 — 앱 전역 「태어났」·「출산 확인」 문자열 0건, 백엔드 subject 전환 라우트 0건, 설정 탭에 배너 없음 | `tbm_dear-baby-docs-impl` (PRD-006) | 출산 확인 팝업 + 전환(AC-006-06·07) 착지 |
| `TC-007-07-B` | 50 도달 시 CTA 전환 | 책 진행도 카운트가 stub — `app/src/api/recordsCount.ts` 가 「백엔드 답변 카운트 API 가 아직 없으므로 결정적 stub」이라 기록 수에 반응하지 않는다(위 「등재 불가 부채」에 이미 기재). 「답변 1개 추가 → 50 도달」·「아이별 누적 30/10」 전제를 세울 수 없다 | `tbm_dear-baby-docs-impl` (PRD-007) | 백엔드 답변 카운트 API + stub 제거 착지 |
| `TC-007-07-C` | 다자녀 — 아이별 독립 카운트 | 책 진행도 카운트가 stub — `app/src/api/recordsCount.ts` 가 「백엔드 답변 카운트 API 가 아직 없으므로 결정적 stub」이라 기록 수에 반응하지 않는다(위 「등재 불가 부채」에 이미 기재). 「답변 1개 추가 → 50 도달」·「아이별 누적 30/10」 전제를 세울 수 없다 | `tbm_dear-baby-docs-impl` (PRD-007) | 백엔드 답변 카운트 API + stub 제거 착지 |
| `TC-008-03-C` | 카드 표시 요소와 미디어 행 | 미디어 행 미구현 — `DiaryCard.tsx` 에 미디어 행 렌더 0건, 첨부 자체가 미구현(TC-005-* 와 같은 부재) | `tbm_dear-baby-docs-impl` (PRD-008 / PRD-005) | 미디어 첨부 + 카드 미디어 행 착지 |
| `TC-008-04-B` | 음성 원본이 있는 기록 상세 | 음성 재생 컨트롤 미구현 — 상세 화면은 「음성 원본 있음」 캡션만(TC-005-05 와 같은 부재) | `tbm_dear-baby-docs-impl` (PRD-008) | 상세 음성 재생 컨트롤 착지 |
| `TC-008-05-C` | 편집 취소 시 변경 폐기 경고 | 편집 취소 시 폐기 확인 모달 미구현 — `app/app/diary/[id]/edit.tsx` 의 취소는 즉시 뒤로 가고(`Alert` 는 저장 실패용 1건뿐) 「저장되지 않았습니다」 문자열 0건 | `tbm_dear-baby-docs-impl` (PRD-008) | 편집 취소 확인 모달 착지 |
| `TC-008-06-B` | 삭제는 해당 아이의 책 진행도만 차감 | 책 진행도가 stub(`recordsCount.ts`)이라 삭제로 차감되지 않는다(TC-007-07-B 와 같은 부재) | `tbm_dear-baby-docs-impl` (PRD-008 / PRD-007) | 백엔드 답변 카운트 API 착지 |
| `TC-008-07-B` | 공개 → 비공개 전환 시 공감 수 유지 | 공감(AC-009-08) 미구현 — likes 테이블·라우트 0건(`CommunityFeedCard.tsx` 이탈 1 「likes 테이블·집계가 없다」) | `tbm_dear-baby-docs-impl` (PRD-008 / PRD-009) | 공감 기능 착지 |
| `TC-008-08-B` | 단일 아이 사용자는 아이 필터 미노출 | 기간·미디어 필터 미구현 — `DiaryFilterSheet.tsx` 머리말 「MVP 범위에서 기간·미디어 필터는 노출하지 않는다」, 시트는 아이 칩 + 공개 여부만 | `tbm_dear-baby-docs-impl` (PRD-008) | 기간·미디어 필터 착지 |
| `TC-008-08-C` | 미디어·기간 필터 조합 | 기간·미디어 필터 미구현 — `DiaryFilterSheet.tsx` 머리말 「MVP 범위에서 기간·미디어 필터는 노출하지 않는다」, 시트는 아이 칩 + 공개 여부만 | `tbm_dear-baby-docs-impl` (PRD-008) | 기간·미디어 필터 착지 |
| `TC-008-09-B` | 필터 결과 0건 빈 상태 | 필터 결과 0건 빈 상태 미구현 — `DiaryEmpty.tsx` 는 「아직 첫 기록이 없어요」 한 종류뿐, 「조건에 맞는 기록이 없어요」·「필터 초기화」 CTA 0건 | `tbm_dear-baby-docs-impl` (PRD-008) | 필터 0건 빈 상태 착지 |
| `TC-009-02` | 메인 화면 구조와 피드 카드 구성 | 커뮤니티 메인은 착지(#172)했으나 ③ 오늘의 질문 카드·공감 수·댓글 수·카드 탭→상세가 없다 — `community.tsx` 머리말 「③ 이번 슬라이스 범위 밖」, `CommunityFeedCard.tsx` 이탈 1·2·4 | `tbm_dear-baby-docs-impl` (PRD-009) | 질문 카드(AC-009-04)·상세(AC-009-07)·공감·댓글 착지 |
| `TC-009-04-A` | 질문 카드 CTA — 미답변 분기 | 커뮤니티 오늘의 질문 카드 미구현 — `community.tsx` 머리말 이탈 1 「③ 오늘의 질문 카드 미구현」(목적지 화면·답변 상태 조회 경로 부재) | `tbm_dear-baby-docs-impl` (PRD-009) | 질문 카드 + CTA 3분기 착지 |
| `TC-009-04-B` | 질문 카드 CTA — 비공개 답변 분기 | 커뮤니티 오늘의 질문 카드 미구현 — `community.tsx` 머리말 이탈 1 「③ 오늘의 질문 카드 미구현」(목적지 화면·답변 상태 조회 경로 부재) | `tbm_dear-baby-docs-impl` (PRD-009) | 질문 카드 + CTA 3분기 착지 |
| `TC-009-04-C` | 질문 카드 CTA — 공개 답변 분기 | 커뮤니티 오늘의 질문 카드 미구현 — `community.tsx` 머리말 이탈 1 「③ 오늘의 질문 카드 미구현」(목적지 화면·답변 상태 조회 경로 부재) | `tbm_dear-baby-docs-impl` (PRD-009) | 질문 카드 + CTA 3분기 착지 |
| `TC-009-05-A` | 모아보기 — 상호 공개 열람 권한 | 같은 질문 답변 모아보기 화면 미구현 — 앱 라우트·백엔드 라우트 0건(`community.tsx` 이탈 1 이 목적지 부재를 명시) | `tbm_dear-baby-docs-impl` (PRD-009) | 모아보기 화면 + 열람 권한 착지 |
| `TC-009-05-B` | 모아보기 — 재비공개 시 권한 즉시 소멸 | 같은 질문 답변 모아보기 화면 미구현 — 앱 라우트·백엔드 라우트 0건(`community.tsx` 이탈 1 이 목적지 부재를 명시) | `tbm_dear-baby-docs-impl` (PRD-009) | 모아보기 화면 + 열람 권한 착지 |
| `TC-009-07-A` | 게시글 상세 — 표시 요소와 본인 글 관리 | 게시글 상세 미구현 — `community.tsx` 이탈 3 「카드 탭 → 상세 이동 없음 — 상세 화면 미구현」, `app/app/community/` 라우트 없음 | `tbm_dear-baby-docs-impl` (PRD-009) | 커뮤니티 상세 화면 착지 |
| `TC-009-07-B` | 공개 수명주기 — 전환 보존·삭제 소멸 | 게시글 상세 미구현 — `community.tsx` 이탈 3 「카드 탭 → 상세 이동 없음 — 상세 화면 미구현」, `app/app/community/` 라우트 없음 | `tbm_dear-baby-docs-impl` (PRD-009) | 커뮤니티 상세 화면 착지 |
| `TC-009-08` | 공감 — 토글과 본인 제한 (경로 무관) | 공감 미구현 — likes 테이블·라우트 0건(`CommunityFeedCard.tsx` 이탈 1) | `tbm_dear-baby-docs-impl` (PRD-009) | 공감 기능 착지 |
| `TC-009-09` | 댓글/대댓글 — 1depth·작성자 표시 | 댓글·대댓글 미구현 — comments 테이블·라우트 0건(`CommunityFeedCard.tsx` 이탈 2) | `tbm_dear-baby-docs-impl` (PRD-009) | 댓글 기능 착지 |
| `TC-009-10` | 개인정보 보호 — 마스킹 표시명과 비노출 항목 | 3단계 「카드·상세 어디에도」의 상세 화면과 4단계의 사진 첨부가 미구현 — 카드의 마스킹 표시명(1·2단계)은 서버가 내려 `CommunityFeedCard` 가 그린다 | `tbm_dear-baby-docs-impl` (PRD-009) | 커뮤니티 상세 + 미디어 첨부 착지 |
| `TC-009-11` | 신고 — 접수·중복 방지·신고자 숨김 | 신고 미구현 — 앱·백엔드에 신고 코드 0건 | `tbm_dear-baby-docs-impl` (PRD-009) | 신고 기능 착지 |
| `TC-009-13` | 빈 상태/예외 상태 문구 | 4단계(삭제·비공개 글 접근 안내)·5단계(모아보기 권한 없음 안내)의 화면이 미구현 — 1~3단계 문구는 `community.tsx` 의 `EMPTY_FEED_TEXT`·`EMPTY_FILTERED_TEXT`·`ERROR_TEXT` 로 착지 | `tbm_dear-baby-docs-impl` (PRD-009) | 커뮤니티 상세 + 모아보기 착지 |
| `TC-009-14-A` | 홈 "다른 엄마들의 기록" 섹션 — 카드 표시 | 3단계 「카드 탭 → 커뮤니티 상세 진입」 미구현 — 홈의 `OtherEntryCard` 는 `app/app/(tabs)/index.tsx` 에서 `onPress` 없이 렌더되고 상세 화면 자체가 없다(더보기 → 커뮤니티 탭 이동은 구현됨) | `tbm_dear-baby-docs-impl` (PRD-009) | 커뮤니티 상세 화면 착지 |
<!-- e2e-pending:end -->

**구현 대기 58건** (TEST-001 1 · TEST-002 7 · TEST-003 4 · TEST-004 4 · TEST-005 8 · TEST-006 10 · TEST-007 2 · TEST-008 8 · TEST-009 14). 부재는 여섯 묶음으로 요약된다 — ① 자서전 탭(AI 서사·책 제작: `memoir.tsx` 플레이스홀더) ② 미디어 첨부·재생 ③ 출산 전환·설정(예정일 수정·아이 추가) ④ 알림(푸시) ⑤ 단계별 질문 풀 ⑥ 커뮤니티 후속(질문 카드·모아보기·상세·공감·댓글·신고) + 낱개(저장 시 공개 토글 #170 · 카운트 stub · 편집 취소 모달 · 기간/미디어 필터 · 필터 0건 빈 상태). 2026-09-08 판정에서 커뮤니티 16건이 「29줄 플레이스홀더」 한 근거로 들어왔으나, #172(커뮤니티 메인, 2026-09-21)가 착지해 `TC-009-01`·`03`·`06` 세 건은 관측 가능해져 공백으로 옮겼고 나머지는 근거를 남은 부재로 좁혔다.

### 공백 목록 (매칭 없음 — 잔존 gap)

> **구현은 돼 있으나 전용 flow 가 아직 없는** 시나리오들이다 — 예외(규칙 4)도 구현 대기(규칙 6)도 아닌 **규칙 1 의 drift** 이며, 이 목록을 한 건씩 줄이는 것이 이 모델의 후속 작업이다. 2026-09-21 전수 판정으로 각 행에 **관측 대상(현재 코드에서 flow 가 단정할 수 있는 화면·컴포넌트·API)** 을 적었다 — 근거가 없는 행은 공백에 둘 수 없다. 재분류가 필요해지는 것은 코드가 바뀔 때뿐이며, 그때는 구현 쪽 변경이 as-is 버전을 움직여 재감지된다.

<!-- e2e-gap:begin -->

| TC | 시나리오 | 관측 대상 (구현 위치) | 분할 후보 |
|----|----------|-----------------------|-----------|
| `TC-001-02` | AI 음성-텍스트 변환 테스트 | `app/app/record-audio-review.tsx` — STT 로딩(`record-audio-review-loading`)·결과 입력(`record-audio-review-input`)·오류(`-stt-error`), whisper.rn 온디바이스 변환 | — |
| `TC-001-03` | 변환 텍스트 편집 테스트 | `record-audio-review.tsx` — 변환 텍스트 편집 가능한 `record-audio-review-input` + 저장(`record-audio-review-save`) | — |
| `TC-001-04` | 텍스트 직접 입력 테스트 | `app/app/record-text.tsx` — 텍스트 입력(`record-text-input`)·저장(`record-text-save`), `POST /records` | — |
| `TC-001-05` | 기록 목록 조회 테스트 | `app/app/(tabs)/diary.tsx` 목록 + `app/app/diary/[id].tsx` 상세 | — |
| `TC-001-07` | 임시 저장 보관·재시도 테스트 | `app/app/drafts.tsx` — 보관함 목록·업로드 재시도(`drafts-upload-*`)·삭제(`drafts-delete-*`), `app/src/drafts/draftStore.ts` | — |
| `TC-002-01-B` | 오늘의 질문 표시 — 양육자 | `HomeQuestionCard.tsx` 컨텍스트 라벨 + `app/src/utils/childLabel.ts` 양육 포맷(`5개월 (152일째)`) | — |
| `TC-002-03-A` | 질문에서 기록 연결 | `HomeQuestionCard` CTA → `record-text.tsx`/`record-audio.tsx` 가 `question_text` 를 실어 `POST /records`(`record-text-question-header`) | — |
| `TC-002-05-A` | 단계 산출 불가 시 폴백 | 온보딩 `a2.tsx` 예정일 건너뛰기(`onboarding-a2-due-date-skip`) → `HomeQuestionCard` 라벨 `null` 이면 배지 미표시 | — |
| `TC-006-01` | 케이스 분기 — 임신 O · 양육 X (Case A) | `app/app/(onboarding)/q1.tsx`·`q2.tsx` 분기 → `a1.tsx` | onboarding-caseA.yaml (Q1 [예] → Q2 [아니요] → a1) |
| `TC-006-02` | 케이스 분기 — 임신 O · 양육 O (Case B) | `q1.tsx`·`q2.tsx` 분기 → `b0.tsx` → `b1.tsx` | onboarding-caseB.yaml (Q1 [예] → Q2 [예] → b0 → b1) |
| `TC-006-03` | 케이스 분기 — 임신 X · 양육 O (Case C) | `q1.tsx`·`q2.tsx` 분기 → `c1.tsx` | onboarding-caseC.yaml (Q1 [아니요] → Q2 [예] → c1) |
| `TC-006-12` | 다자녀 — 홈 상단 아이 전환 탭 노출 조건 | `HomeHeader.tsx` — `canNavigate` 로 단일 아이면 화살표 비활성(`arrowDisabled`)·다자녀면 활성 | — |
| `TC-006-13` | 다자녀 — 화살표 탭으로 활성 컨텍스트 전환 및 순환 | `ActiveChildContext.tsx` `next`/`prev` 가 `% list.length` 로 순환, 임신·양육 단일 리스트 | — |
| `TC-006-15` | 다자녀 — 활성 아이가 세션 간 유지 | `ActiveChildContext.tsx` — 활성 인덱스를 `AsyncStorage` 에 영속화·복원 | — |
| `TC-007-01-B` | 임산부 — D-day 표시 (잔여일 100일 이하) | `childLabel.ts` `formatPregnancyLabel` — 잔여일 ≤ 100 이면 `D-NN` | home-header-active-child.yaml (라벨 정규식이 `D-NN` 도 허용) |
| `TC-007-01-C` | 양육자 — 개월 수/나이 자동 분기 | `childLabel.ts` — 누적 개월 ≤ 12 `mm개월 (nn일째)` / ≥ 13 `nn살 (nn일째)` | — |
| `TC-007-02-A` | 단일 아이 — 화살표 비활성화 | `HomeHeader.tsx` — 단일 아이면 `disabled` + `arrowDisabled`(opacity 0.35) | — |
| `TC-007-06-B` | 텍스트 기록 진입 | `HomeQuestionCard` 텍스트 CTA(`home-text-cta`) → `record-text.tsx` 모달, 질문 동봉 저장 | home-record-ctas.yaml (`home-text-cta` → `record-text-screen`) |
| `TC-007-06-C` | 회전된 질문 기준 저장 | `HomeQuestionCard` 회전 → `record-text.tsx` 가 현재 표시 질문(`question_text`)을 저장 | — |
| `TC-008-01-A` | 단일 아이 — 모든 기록 통합 표시 | `diary.tsx` 통합 목록 + `DiaryHeader.tsx`(타이틀·종, 화살표 없음) | — |
| `TC-008-01-C` | 활성 아이 변경이 일기 탭에 영향 없음 | `diary.tsx` 가 활성 아이와 무관하게 `GET /records` 전체를 렌더 | — |
| `TC-008-02-A` | 월 단위 시간 그룹 헤더 | `diary.tsx` `SectionList` 월별 섹션 + `stickySectionHeadersEnabled` | — |
| `TC-008-02-B` | 아이별 컨텍스트는 그룹 키가 아님 | `diary.tsx` 섹션 키가 월(YYYY년 MM월)뿐, 아이 컨텍스트는 `ChildContextChip` 으로 카드 안에 | — |
| `TC-008-03-A` | 기록 카드 — 아이 컨텍스트 칩 표시 | `DiaryCard.tsx` — `ChildContextChip` + `VisibilityBadge` | — |
| `TC-008-03-B` | 단일 아이 사용자도 아이 컨텍스트 칩 표시 | `DiaryCard.tsx` — 아이 수와 무관하게 칩 렌더 | — |
| `TC-008-05-B` | 다른 아이로 옮기기 불가 | `app/app/diary/[id]/edit.tsx` — 본문 textarea 만 편집, 아이 선택 UI 없음 | — |
| `TC-008-06-C` | 공개 기록 삭제 시 커뮤니티 피드 즉시 제거 | `DELETE /records/{id}` + `GET /community/feed` 가 실재 행만 반환(`community.go` `visibility='public'` 절) | — |
| `TC-008-08-D` | 필터 초기화 정책 | `diary.tsx` 머리말 「필터 시트는 화면-로컬 useState — 일기 탭 이탈 시 초기화」 — 초기화가 실제로 일어나는지가 flow 의 첫 단정 대상 | — |
| `TC-008-10-B` | 하단 네비게이션 — 일기 탭 선택 상태 | `app/app/(tabs)/_layout.tsx` 5탭 + `diary-tab` 도착 | diary-header-nav.yaml (헤더 주석이 TC-008-10-B 까지 경유를 명시) |
| `TC-009-01` | 콘텐츠 출처 — 공개 기록만, 글쓰기 없음 | `community.tsx` — `GET /v1/community/feed` 가 `visibility='public'`·타인 기록만(`community.go`), 화면에 글쓰기 진입점 0건 | — |
| `TC-009-03` | 상단 상태값 표시와 아이 전환 갱신 | `community.tsx` `community-stage-pill`(`communityStageLabel.ts`) + 아이 전환 시 피드 재조회, 수동 필터 없음(View 로 렌더) | — |
| `TC-009-06` | 콘텐츠 타입 필터 | `CommunityTypeFilter.tsx` 전체/질문답변/자유일기 → 서버 `type` 쿼리(`FeedTypeQuestion`/`FeedTypeDiary`) | — |
<!-- e2e-gap:end -->

**공백 32건** (TEST-001 5 · TEST-002 3 · TEST-006 6 · TEST-007 5 · TEST-008 10 · TEST-009 3). **분할 후보 6건**(「분할 후보」 열)은 기존 flow 가 이미 커맨드로 지나가지만 선언하지 않은 시나리오다(규칙 2 상 한 파일은 1개만 선언한다). 새 셋업이 필요 없어 **가장 싼 후속**이므로 먼저 집을 것을 권한다 — 경유 구간을 잘라 전용 flow 로 세우고 원 flow 는 자기 선언만 남긴다.


### 집계

- 테스트 시나리오: **112개** (`docs/tests/` 9문서 — TEST-001 7 · 002 11 · 003 4 · 004 4 · 005 8 · 006 19 · 007 15 · 008 26 · 009 18)
- 매칭 파일: **22개** / 예외: **0건** / 구현 대기: **58건** / 공백: **32건**
- 불변식 (112 − 0 − 58) = **54** ≠ **22** → **부족 32건** (= 공백 목록)
- 매칭 단위 파일: 최상위 `e2e/maestro/*.yaml` **24개** = 시나리오 매칭 22 + 엔지니어링 노트 1(`login.yaml`) + 스모크 예외 1(`health.yaml`). `subflows/*.yaml` 9개는 매칭 단위가 아니다.
- 기계 검증: `bash e2e/scripts/check-scenario-mapping.sh` — 위 다섯 수를 레포에서 다시 계산해 이 문서와 대조하고, 112개 ID 가 매칭·예외·구현 대기·공백 네 표에 **정확히 한 번씩** 나타나는지(누락·중복 0)까지 확인해 어긋나면 비-0 으로 종료한다.
...
- PRD-007 의 확정 AC(8개)는 TEST-007 의 15개 TC 로 커버됨 (구 08/09 검증은 TEST-009·ENG로 이관)
- PRD-008 의 모든 AC(10개)는 TEST-008 의 26개 TC 로 커버됨
...
|------|-----------|-----------|-----------|
| 2026-09-21 e2e 판정 축 전환 (AC → 테스트 시나리오) + 112건 전수 분류 | **`docs/tests` 의 테스트 시나리오(`TC-NNN-NN`)를 e2e 1:1 매칭의 판정 단위로 전환** (reconciler `rct_20260908-0001`). ① **선언 규약 이관** — 제품 flow 22 개의 헤더 `검증 대상: AC-…` 를 **`검증 시나리오: TC-…`** 로 바꾸고 소속 AC 는 `상위 AC:` 로 병기했다. 비-시나리오 허용 유형 2 개(`login.yaml`→ENG-003, `health.yaml`→스모크)는 `검증 대상:` 키를 유지해 **두 키의 파일 집합이 서로소**가 되게 했다 — 이제 `검증 시나리오:` 줄 수가 곧 매칭 파일 수다. ② **등재 표 대체** — `AC ↔ e2e flow 매핑`(69 행) 표를 `테스트 시나리오 ↔ e2e flow 매핑`(22 행)으로 대체하고 **상위 AC 를 열로 보존**해 AC 커버리지가 같은 표에서 파생되게 했다. flow 가 더 이상 AC 를 선언하지 않으므로 AC 표를 남기면 **선언 주체 없는 낡은 SSOT** 가 된다. ③ **예외 / 구현 대기 표 분리** — 영구 면제(예외 0 건, 모킹 경계 레지스트리로 위임하는 기존 입장 승계)와 임시 보류(구현 대기)를 별도 표로 나눴다. ④ **112 건 전수 분류 — 시나리오별 코드 근거로** — 112 = 매칭 22 + 예외 0 + 구현 대기 58 + 공백 32, 어느 TC 도 두 칸에 걸치지 않는다. 기준은 「테스트 단계 중 하나라도 존재하지 않는 화면·기능·API 를 요구하면 구현 대기, 전 단계가 현재 코드로 관측 가능하면 공백(규칙 1 drift)」이고, 구현 대기 58 행 각각에 부재의 코드 위치·담당·해제 조건을, 공백 32 행 각각에 관측 대상(구현 위치)을 적었다. 2026-09-08 초안이 커뮤니티 16 건만 구현 대기로 두고 74 건을 미판정 공백으로 남겼던 것을, main `1c7a1cd`(#172 커뮤니티 메인 착지 포함) 기준 앱·백엔드 전수 대조로 다시 갈랐다 — 커뮤니티 `TC-009-01`·`03`·`06` 은 #172 로 관측 가능해져 공백으로, 자서전(TEST-003·004)·미디어(TEST-005)·출산 전환/설정/알림(TEST-006 10 건)·단계별 질문 풀(TEST-002) 등 42 건은 근거와 함께 구현 대기로. 공백 32 건 중 기존 flow 가 이미 경유하는 **분할 후보 6 건**을 별도 표시(가장 싼 후속). ⑤ **계수 정정 2 건** — (a) `docs/tests` 의 `### ` 헤딩은 113 개지만 하나(`TC-007-08 / TC-007-09-A / TC-007-09-B *(TEST-009로 이관)*`)는 **이관 묘비**라 판정 단위 형태(`TC-NNN-NN: 제목`)가 아니다 → 시나리오 실수는 **112**. (b) 낡아 있던 TC 수 갱신: TEST-007 14→**15**, TEST-008 22→**26**. ⑥ **기계 가드 신설** — `e2e/scripts/check-scenario-mapping.sh` 가 선언 유일성·TC 실재·중복·키 분리·집계 일치에 더해 **네 표(매칭·예외·구현 대기·공백)의 ID 집합이 시나리오 전수를 정확히 한 번씩 덮는지**를 레포에서 재계산해 어긋나면 비-0 으로 죽는다(집계가 조용히 낡는 것이 이 축의 고질). 주석·문서·정적 스크립트 전용 변경 — **flow 커맨드·앱 코드·CI 배선(run-list·워크플로) 무변경**, AC 계층 수치(AC 70·PRD 9)와 모킹 경계 레지스트리 무변동. ⚠️ 범위 밖(후속): 공백 32 건의 flow 신설(분할 후보 6 건부터), 구현 대기 58 건의 구현(`tbm_dear-baby-docs-impl`), #170 착지 시 `TC-001-06` 의 매칭 전환. | 판정 축 AC(69), flow 선언 `검증 대상: AC-…` 22 건, AC↔flow 표 ✅22·⬜47, 시나리오 축 등재 0, 기계 가드 없음 | 판정 축 **테스트 시나리오(112)**, flow 선언 `검증 시나리오: TC-…` **22 건**(중복 0·누락 0), TC↔flow 표 22 행 + 예외 0 + 구현 대기 58(근거 등재) + 공백 32(관측 대상 등재) (합 112), 체커 1개 |
| 2026-08-15 여정 ↔ 목업 매핑 레지스트리 신설 | 여정 ↔ 목업 1:1 정합성의 **매핑 SSOT 부재** 해소 (reconciler rct_20260815-0001): ① **`docs/journey-mockup-map.md` 신설** — 여정 6개 ↔ 진입점 6개(갤러리 그룹 5 + 독립 여정 페이지 1, 규칙 1·2)와 단계 25개 ↔ 카드 43장을 전수 등재하고, 미시각화 단계 5건·고아 진입점 1건·구 번호 라벨 38장을 각각 상한이 선언된 원장으로 고정했다. ② **`docs/mockups/README.md` 일람 표의 `여정 / Stage` 칸을 여정 문서 기준으로 정정** — 43행 중 36행이 여정 문서에 **존재하지 않는 식별자**를 가리키고 있었다. 가장 위험한 형태는 여정을 가로지르는 충돌로, M-31~M-34(Book Production)가 `9-1`~`9-4` 를 인용했는데 그 번호는 문서상 AI Narrative 의 것이다(Book Production 은 `10-1`~`11`). 같은 절의 선언 `✅ M-01~M-42 는 6개 사용자 여정 stage 와 1:1 매핑` 은 **사실이 아니었으므로** 실제 상태(전수 그룹 매핑 ✅ · 미시각화 5 · 구 번호 라벨 38 · 고아 1)로 교체했다. ③ **「여정 mockup 예외」 섹션 신설**(등재 0건) — 모델 규칙 8 의 등재 지점이 없어 "예외 등재 없이 진입점 없는 여정"과 "정당한 예외"를 구분할 수 없던 상태를 닫았다. ④ **`scripts/check-journey-mockup-map.mjs` + `.github/workflows/journey-mockup-map.yml` 신설** — J0~J10 을 PR·main push 마다 강제한다(원장은 래칫: 새 위반도, 고쳐졌는데 남은 공전 행도 실패). 번들 동기화 검사가 "소스만 고치고 재빌드 안 함"을 잡고, J10 은 독립 여정 페이지를 가진 여정의 단계별 카드를 페이지 소스와 직접 대조한다. ⑤ **머지 전 재기준(2026-09-20)** — 준비 이후 main 이 여정 문서를 `docs/user-journeys/` 로, 갤러리 번들을 `docs/mockups/index.html` 로 옮기고 `diary-browse` 를 슬러그 단계 + 독립 여정 페이지로 이관해 체커가 머지 트리에서 위반 106건을 냈다. 경로 상수·단계 파서·진입점 모형을 그 실측에 맞춰 고쳤다(단계 총수 25·여정 6·카드 43 은 이동 없음). 문서·검사 전용 변경 — 목업 소스·번들 바이트·앱 코드·AC↔flow 매핑 무변경, 집계 무이동. | 매핑 SSOT 없음(`docs/journey-mockup-map.md` 부재), README 일람 표의 Stage 인용 36행이 문서에 없는 식별자, 거짓 1:1 선언, 여정 mockup 예외 등재 지점 없음, 기계 검증 0 | 매핑 SSOT 신설(여정 6·단계 25·카드 43 전수), README Stage 인용 43행 전부 문서에 실재, 원장 3종(5·1·38) 상한 선언, 예외 섹션 신설(등재 0건), J0~J10 CI 강제 |
