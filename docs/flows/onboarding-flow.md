...
flowchart TD
    ...
    A1 --> A2["태아 정보 입력<br/>태명(선택) · 성별(미정 포함)<br/>임신 주차 · 예정일<br/>(다태인 경우 태아 수만큼 A2 인스턴스 반복)"]
    A2 --> A3["일기 목적<br/>복수 선택<br/>(다태에서도 1회만, 모든 태아에 동일 적용)"]
    A3 --> HomeEnter([홈 진입])

    %% ===== Case B =====
    CaseB --> B0(["① 양육 중인 아이 먼저"])
    B0 --> B1["양육 중 아이 수 입력<br/>1명 / 2명 / 3명 이상"]
    B1 --> B2["각 아이 정보 입력<br/>이름 · 성별 · 생년월일<br/>한줄 소개 · 사진(선택)"]
    B2 --> B2P["각 아이 일기 목적 입력<br/>양육 톤 칩 8 종 다중 선택<br/>(B2 ↔ B2-Purpose 가 아이 수만큼 반복)"]
    B2P --> B3(["② 임신 중인 아이 이어서"])
    B3 --> B4["임신 중 아이 수 입력<br/>단태 / 다태"]
    B4 --> B5["태아 정보 입력<br/>태명 · 성별 · 임신 주차 · 예정일<br/>태아 수만큼 반복"]
    B5 --> B6["일기 목적<br/>임신 톤 칩 8 종 다중 선택<br/>(다태에서도 1회만, 모든 태아에 동일 적용)"]
    B6 --> HomeEnter

    %% ===== Case C =====
    CaseC --> C1["양육 중 아이 수<br/>1명 / 2명 / 3명+"]
    C1 --> C2["아이 정보 입력<br/>이름 · 성별<br/>생년월일<br/>한줄 소개 · 사진"]
    C2 --> C3["각 아이 일기 목적 입력<br/>양육 톤 칩 8 종 다중 선택<br/>(C2 ↔ C3 가 아이 수만큼 반복)"]
    C3 --> HomeEnter
    ...
    class CaseB,B0,B1,B2,B2P,B3,B4,B5,B6 caseB
...
- 각 아이마다 정보(이름·성별·생년월일·한줄 소개·사진) → 일기 목적(양육 톤 칩) 을 1:1 화면 한 쌍(B2 ↔ B2-Purpose)으로 입력하고, 아이 수만큼 반복
...
**③ 일기 목적 (태아)**
- 임신 톤 칩 8 종 단일 그리드에서 다중 선택 — 다태에서도 1회만 묻고 모든 태아 행에 동일하게 적용 (Case A 와 같은 모델, 태아별 개별 선택은 후속 작업)
- 양육 아이의 일기 목적은 ① 단계에서 아이별 1:1 화면(B2-Purpose)으로 이미 받았으므로 본 화면에는 노출되지 않음
...
- 양육 중 아이 수
- 각 아이마다 정보(이름·성별·생년월일·한줄 소개·사진) → 일기 목적(양육 톤 칩) 을 1:1 화면 한 쌍(C2 ↔ C3)으로 입력하고, 아이 수만큼 반복
- 마지막 아이의 C3 [시작하기] 에서 모든 아이 행을 한 번에 영속화한다 (Case B 의 양육 입력과 같은 모델)
