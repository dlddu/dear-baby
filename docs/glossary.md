...
| 아이 단계 *(제안)* | `ChildStage` (`{ kind: 'pregnancy'; days: number } \| { kind: 'postnatal'; days: number; months: number }`) | `stage`, `childStage` | — (파생 값) | 아이가 지금 어느 시기에 있는지. **임신 주차 축과 생후 나이 축 중 정확히 하나**에 속하며 두 값을 동시에 갖지 않는다. 출산 전환이 축을 바꾸는 유일한 사건. 산출 불가 시 `null` (= 단계 없음). 값은 축마다 다르다 — 임신은 `days` = daysPregnant (주차 = `/7`, 일수 = `%7`), 양육은 `days` = daysOld 와 **달력 기준** `months` **둘 다** (개월은 일수에서 복원되지 않는다 — ENG-013) | PRD-002 질문 모델, ENG-001, ENG-011, ENG-013 |
...
| 생후 나이 *(제안)* | `PostnatalAge` (number, 일 단위) | `postnatalAge`, `daysOld` | `birth_date` 에서 파생 · 기록에는 `records.stage_days` 로 스냅샷 | 생년월일 기반으로 계산된 경과 시간. `ChildStage` 의 양육 축. **저장은 0-based** (출생 당일 = `0`) 이고 **표기가 +1** 이다 — "생후 1일째". 개월·연 환산은 30일 나눗셈이 아니라 **달력 기준**이며 말일 출생은 그 달 마지막 날로 절사한다. 상한 없음 | PRD-002, AC-002-02, PRD-007 AC-007-01, ENG-001, ENG-013 |
| 단계 스냅샷 *(제안)* | `StageSnapshot` (`ChildStage \| null`) | `stageAtWriting` | `records.stage_kind` (`'pregnancy'\|'postnatal'`) · `records.stage_days` · `records.stage_months` — **셋 다 nullable 이며 함께 움직인다** (마이그레이션 0013) | 기록·답변이 **작성될 당시**의 아이 단계. 저장 시점에 계산해 **물리 저장**한다. 시간 경과·출산 전환으로는 불변이나, **기준값(예정일·출생일) 정정 시 재계산**된다 (ENG-013). 단계 산출 불가 상태로 작성된 기록은 세 컬럼 모두 `null`. `stage_kind` 가 `record_subjects.kind`(`'fetus'\|'child'`) 와 어휘가 다른 것은 의도된 것이다 — 스냅샷의 축은 아이의 *현재* 축과 독립이다 | PRD-002 AC-002-03, ENG-011, ENG-013 |
