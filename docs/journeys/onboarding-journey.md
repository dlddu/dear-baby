...
| 🌼 **Case B** | 8 (B0·B1·B2·B2-Purpose·B3·B4·B5·B6) | "양 많은데 단계가 잘 나눠져 있어서 견딜 만하다 — 아이마다 어떤 기록을 남길지도 따로 물어봐 주는 게 세심하다" | B0/B3 안내 화면이 양육·임신 두 단계를 명시적으로 끊어줌 (단계 인디케이터 ①→②) · 양육 아이는 정보 입력 직후 1:1 (B2-Purpose) 로, 태아는 마지막에 일괄 (B6) 로 기록 목적을 받아 인지 부담 분산 | AC-006-03 |
| 🌿 **Case C** | 3 (C1~C3) | "임신 질문 거치지 않아도 되는 게 다행" | (Case A와 동일한 입력 허들 완화 장치) | AC-006-04 |
...
| 4 | Stage 4 (Case B) | 8화면 — 중도 이탈 위험 (다자녀일 경우 더 길어짐: 양육 N명 → B2 + B2-Purpose × N) | B0·B3 안내 화면 + 단계 인디케이터 ①→② · 양육 1:1, 태아 일괄로 기록 목적 입력 부담 분산 | 입력 진행률 % 표시, "이만큼 남았어요" 카피 |
